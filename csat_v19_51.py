...
주요 변경:
  ...
  - 프롬프트 구조 재편 (가이드 → 시각 → 깊이 → 보안 → 증거 → OCR → 계약)
...
def pre_classify(text: str, filename: str = "") -> dict:
    items = []
    tn = _norm(text)
    items += _scan_keywords(tn, META_HINT, "anti", ["전체"], "표지/해설 → 미분류 가능성", 0.85, "context")
    items += _scan_keywords(tn, NON_CSAT_HINT, "anti", ["전체"], "비수능 컨텐츠 가능성", 0.7, "context")
